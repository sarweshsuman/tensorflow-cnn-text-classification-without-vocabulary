...
for file in os.listdir(FLAGS.data_dir):
    data_files.append(os.path.join(FLAGS.data_dir,file))

x_text, y = data_helpers.load_data_and_labels(data_files)

max_document_length = max([len(word_tokenize(x)) for x in x_text])
print("Max length of a sentence {}".format(max_document_length))
...
print("converting training data into array of vectors")
x_train=[]
...
for ln in x_text:
	...
	for i in range(max_document_length-len(words)):
		arr_of_vectors.append(zero_vec)
	x_train.append(arr_of_vectors)

x_numpy=np.array(x_train,dtype='float32')

# Randomly shuffle data
np.random.seed(10)
shuffle_indices = np.random.permutation(np.arange(len(y)))
x_shuffled = x_numpy[shuffle_indices]
y_shuffled = y[shuffle_indices]

""" Due to small data set, i will ignore creating dev set of data """
